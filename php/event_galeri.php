...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    ...
    if ($id > 0) {
        ...
        if ($img_data = mysqli_fetch_assoc($img_result)) {
            ...
            header("Location: event_galeri.php?status=error&message=" . urlencode("Data tidak ditemukan"));
            exit;
        }
    } else {
        header("Location: event_galeri.php?status=error&message=" . urlencode("ID tidak valid"));
        exit;
    }
}
...
<body>
        ...
        <div class="content">
            ...
            <div class="main-content">
                <div class="section-header">
                    ...
                    <a href="tambah_event_galeri.php" class="btn btn-primary btn-tambah">
                        <i class="fas fa-plus"></i> Tambah Event Galeri
                    </a>
                </div>
                    ...
                    <?php if (mysqli_num_rows($result) > 0): ?>
                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                            <div class="gallery-card">
                                <?php
                                $imagePath = "uploads/" . htmlspecialchars($row['gambar']);
                                ...
                                <div class="gallery-actions">
                                    <button class="btn-edit" onclick="window.location.href='edit_event_galeri.php?id=<?php echo $row['id']; ?>'">
    ...
    function confirmDelete(id) {
        if (confirm('Apakah Anda yakin ingin menghapus event galeri ini?')) {
            window.location.href = 'event_galeri.php?action=delete&id=' + id;
        }
    }
    </script>
</body>
