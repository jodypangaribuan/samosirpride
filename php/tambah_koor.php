...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $user_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 0; // default 0 jika tidak ada session

    if (empty($nama) || empty($tempat) || empty($waktu)) {
        $status = "error";
        $message = "Semua field harus diisi.";
    } else {
        // Masukkan ke database
        $query = "INSERT INTO koor (nama, tempat, waktu_latihan, user_id) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssi", $nama, $tempat, $waktu, $user_id);
...
<div class="container">
    <div class="header">
        ...
        <a href="koor.php" class="back-link"><i class="fas fa-arrow-left"></i> Kembali</a>
    </div>
    ...
    <form method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="tempat" id="tempat" value="<?php echo isset($_POST['tempat']) ? htmlspecialchars($_POST['tempat']) : ''; ?>" required>
        </div>
        ...
        <div class="button-group">
            ...
            <a href="koor.php" class="btn cancel-btn"><i class="fas fa-times"></i> Batal</a>
        </div>
    </form>
</div>
</body>
